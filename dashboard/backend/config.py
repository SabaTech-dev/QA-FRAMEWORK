...
class Settings(BaseSettings):
    ...
    # CORS - explicit allowlist of trusted origins (comma-separated env var).
    # Wildcard origins are insecure with credentials; never use "*".
    cors_origins: list[str] = [
        origin.strip()
        for origin in os.getenv(
            "CORS_ORIGINS",
            "https://qa.sabatech.dev,http://localhost:3000,http://localhost:3010",
        ).split(",")
        if origin.strip()
    ]
    ...
    def _warn_insecure_defaults(self):
        ...
        if self.ENVIRONMENT != "production":
            ...
            if not self.secret_key:
                ...
                warning_messages.append(
                    "JWT_SECRET_KEY not set - using dev fallback (sessions will not persist across restarts)"
                )
