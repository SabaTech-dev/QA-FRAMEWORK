...
import xmlrpc.client  # nosec B411 — TestLink exposes its official API via XML-RPC; server is operator-configured, not untrusted input
import time
...
class TestLinkIntegration(IntegrationBase):
    ...
    async def sync_test_results(
        ...
        **kwargs,
        ...
        if not project:
            return SyncResult(
                provider=self.provider_name, success=False, errors=["No project specified"]
            )
        ...
        try:
            ...
            return SyncResult(
                provider=self.provider_name, success=False, errors=[f"Project not found: {project}"]
            )

        # Create test plan if needed
        plan_name = cycle_name or f"QA-Framework Plan {datetime.now().strftime('%Y-%m-%d %H:%M')}"
        try:
            plan_response = self._client.tl.createTestPlan(self.api_key, plan_name, project)
        ...
        for result in results:
            try:
                ...
                try:
                    ...
                    search_result = self._client.tl.getTestCaseIDByName(result.test_name, project)
                    if search_result:
    ...
    async def get_test_cases(
        ...
        for suite in suites:
            ...
            tcs = client.tl.getTestCasesForTestSuite(self.api_key, suite["id"], True, "full")
    ...
    async def get_projects(self) -> List[Dict[str, Any]]:
        ...
        return [{"key": str(p["id"]), "name": p["name"], "id": p["id"]} for p in projects]
    ...
    def get_config_schema(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "properties": {
                ...
                "default_project": {"type": "string", "title": "Default Project Name"},
                "dev_key": {"type": "string", "title": "Developer Key (Optional)", "secret": True},
            },
