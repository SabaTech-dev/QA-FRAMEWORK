...
from core.smart_cache import CacheStrategy, CacheConfig, SmartCache, get_cache
...
class TestSmartCacheInit:
    ...
    def test_init_custom_params(self, mock_redis):
        ...
        mock_redis.assert_called_once_with(host="127.0.0.1", port=6380, db=1, decode_responses=True)
    ...
    def test_init_stats_initialization(self, mock_redis):
        ...
        assert cache.stats == {"hits": 0, "misses": 0, "invalidations": 0, "sets": 0}
...
class TestSmartCacheKeyGeneration:
    ...
    def test_generate_key(self, mock_redis):
        ...
        assert len(key1) == len("qa:test:") + 32  # 128-bit blake2b hash (hex)
...
class TestSmartCacheInvalidate:
    ...
    def test_invalidate_dependent(self, mock_redis_class):
        ...
        mock_redis = Mock()
        real_key = f"qa:test:{hashlib.blake2b('base'.encode(), digest_size=16).hexdigest()}"
...
class TestSmartCacheStats:
    ...
    def test_get_stats_basic(self, mock_redis_class):
        ...
        mock_redis = Mock()
        mock_redis.info.return_value = {"used_memory": 1024000, "used_memory_human": "1.00M"}
        mock_redis_class.return_value = mock_redis
...
class TestSmartCacheWarmCache:
    ...
    def test_warm_cache(self, mock_redis_class):
        ...
        data = {"id1": {"value": 1}, "id2": {"value": 2}, "id3": {"value": 3}}
        config = {"prefix": "test", "ttl": 300, "tags": ["test"]}
...
class TestSmartCacheClearAll:
    ...
    def test_clear_all(self, mock_redis_class):
        """Test clear_all."""
        mock_redis = Mock()
        mock_redis.scan_iter.return_value = ["qa:test:key1", "qa:test:key2", "qa:tags:tag1"]
        mock_redis.delete.return_value = 1
        mock_redis_class.return_value = mock_redis
