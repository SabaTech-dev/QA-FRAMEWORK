...
def mock_test_case():
    """Mock test case"""
    return TestCase(
        id=1, suite_id=1, name="Test Case 1", test_code="def test_example(): pass", is_active=True
    )
...
def mock_test_suite(mock_test_case):
    ...
    suite = TestSuite(
        id=1, name="Test Suite 1", description="Test suite", framework_type="pytest", created_by=1
    )
...
def mock_execution_detail():
    ...
    return TestExecutionDetail(id=1, execution_id=1, test_case_id=1, status="pending")
...
class TestCreateExecutionService:
    ...
    async def test_create_execution_success(self, mock_db, mock_test_suite):
        ...
        execution_data = TestExecutionCreate(
            suite_id=1, execution_type="manual", environment="production"
        )
    ...
    async def test_create_execution_suite_not_found(self, mock_db):
        ...
        execution_data = TestExecutionCreate(
            suite_id=999, execution_type="manual", environment="production"
        )
    ...
    async def test_create_execution_with_inactive_tests(self, mock_db):
        ...
        with patch("services.execution_service.cache_manager") as mock_cache_mgr:
            ...
            added_details = []
            mock_db.add.side_effect = lambda obj: (
                added_details.append(obj) if isinstance(obj, TestExecutionDetail) else None
            )
...
class TestStartExecutionService:
    ...
    async def test_start_execution_success(self, mock_db, mock_execution):
        ...
        with (
            patch("services.execution_service.get_execution_by_id", return_value=mock_execution),
...
class TestRunTests:
    ...
    async def test_run_tests_success(
        self, mock_db, mock_execution, mock_test_case, mock_execution_detail
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_tests_empty_suite_fails_explicitly(self, mock_db, mock_execution):
        """Anti false-green: a suite that executed 0 assertions must FAIL, not pass.

        Regression guard for the 'green suite that never ran' failure class
        (workboard card a34a1b5c): 0 executed tests treated as success.
        """
        mock_execution.details = []  # empty suite: nothing to execute
        mock_execution.total_tests = 0

        mock_exec_result = AsyncMock()
        mock_exec_result.scalar_one = Mock(return_value=mock_execution)
        mock_db.execute.side_effect = [mock_exec_result]
        ...
        with patch("services.execution_service.cache_manager") as mock_cache_mgr:
            ...
            assert mock_execution.status == "failed", (
                f"Suite with 0 executed assertions must fail, got status={mock_execution.status!r}"
            )
            # And the failure must say explicitly WHY: never ran, not ran-and-passed
            summary = mock_execution.results_summary or {}
            assert "0" in str(summary.get("error", "")), (
                f"results_summary must carry an explicit 'never ran' error, got {summary!r}"
            )

    @pytest.mark.asyncio
    async def test_run_tests_with_failures(self, mock_db, mock_execution, mock_test_case):
        """Test test execution with failures"""
        detail = TestExecutionDetail(id=1, execution_id=1, test_case_id=1, status="pending")
...
class TestListExecutionsService:
    ...
    async def test_list_executions_with_status_filter(self, mock_db, mock_execution):
        ...
        with patch("services.execution_service.cache_manager") as mock_cache_mgr:
            mock_cache_mgr.async_get = AsyncMock(return_value=None)
            mock_cache_mgr.async_set = AsyncMock()
            mock_cache_mgr.get_execution_list_key = Mock(
                return_value="executions:list:status:running"
            )
    ...
    async def test_list_executions_with_pagination(self, mock_db, mock_execution):
        ...
        with patch("services.execution_service.cache_manager") as mock_cache_mgr:
            ...
            mock_cache_mgr.async_set = AsyncMock()
            mock_cache_mgr.get_execution_list_key = Mock(
                return_value="executions:list:skip:10:limit:20"
            )
