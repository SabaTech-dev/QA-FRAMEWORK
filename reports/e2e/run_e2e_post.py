#!/usr/bin/env python3
"""
QA-FRAMEWORK E2E — Post-Onboarding Verification
"""
...
RESULTS_FILE = os.path.join(SCREENSHOT_DIR, "e2e_post_onboard.json")
...
with ip as browser:
    ...
    errs = []
    def on_console(msg):
        if msg.type == "error": errs.append(msg.text[:200])
    ...
    print(f"\n🧪 QA-FRAMEWORK E2E (Post-Onboarding)\n")
    ...
    try:
        ...
        time.sleep(2)
        s1 = ss(page, "v2_01_login")
        rec("1a. Login Page", "pass", "Login form visible", s1)
        ...
        url = page.url
        s2 = ss(page, "v2_01_after")
        
        if "/dashboard" in url:
            rec("1b. Login → Dashboard", "pass", f"Redirected to {url}", s2)
        elif "/onboarding" in url:
            rec("1b. Login → Dashboard", "warn", f"Redirected to onboarding (expected: dashboard)", s2)
        else:
            rec("1b. Login → Dashboard", "fail", f"Unexpected URL: {url}", s2)
    ...
    try:
        ...
        s = ss(page, "v2_02_dashboard")
        
        if "/login" in page.url:
            rec("2. Dashboard", "fail", "Redirected to login", s)
        elif words > 50:
            rec("2. Dashboard", "pass", f"Dashboard loaded: {words} words, {cards} cards", s)
        elif words > 20:
            rec("2. Dashboard", "warn", f"Dashboard loaded: {words} words (minimal)", s)
        else:
        ...
        if "/login" in page.url:
            ...
            if create_sel:
                ...
                if suite_name.lower() in body2.lower():
                    ...
                    rec("3. Create Suite", "warn", f"Form submitted, suite not confirmed in list", s3)
            else:
                # Check if it's the onboarding page
                if "welcome" in body.lower() or "onboarding" in body.lower() or "let's get started" in body.lower():
                    rec("3. Create Suite", "warn", "Onboarding page shown instead of suites", s)
                else:
                    rec("3. Create Suite", "warn", f"No create button. Page: {body[:150]}", s)
    except Exception as e:
        rec("3. Create Suite", "fail", str(e)[:200])

    # === CHECK 4: CRUD ===
    print("\n"+"="*60); print("CHECK 4: CRUD"); print("="*60)
    try:
        page.goto(f"{BASE_URL}/suites", wait_until="networkidle", timeout=30000)
        time.sleep(2)
        ...
        s = ss(page, "v2_04_crud")
        print(f"  Rows: {rows}, Edit/Delete: {edit}")
        
        if edit > 0:
            rec("4. CRUD", "pass", f"{rows} rows, {edit} edit/delete actions", s)
        elif rows > 0:
            rec("4. CRUD", "warn", f"{rows} rows but no visible actions", s)
        else:
            rec("4. CRUD", "warn", "No data visible", s)
    except Exception as e:
    ...
    try:
        ...
        s = ss(page, "v2_05_execute")
        if run > 0:
            page.locator("button:has-text('Run')").first.click()
            time.sleep(5)
            s2 = ss(page, "v2_05_after")
            ...
            rec("5. Execute", "warn", "No run button found", s)
    except Exception as e:
    ...
    try:
        ...
        s = ss(page, "v2_06_reports")
        print(f"  Words: {words}, Keywords: {found[:5]}")
        
        if "/login" in page.url:
            rec("6. Reports", "fail", "Redirected to login", s)
        elif found and words > 20:
            rec("6. Reports", "pass", f"Reports loaded: {words} words", s)
        elif words > 10:
            rec("6. Reports", "warn", f"Reports page loaded ({words} words)", s)
        else:
            rec("6. Reports", "warn", f"Reports empty ({words} words)", s)
    except Exception as e:
        rec("6. Reports", "fail", str(e)[:200])

    # === CHECK 7: Navigation ===
    print("\n"+"="*60); print("CHECK 7: Navegación entre Vistas"); print("="*60)
    try:
        ...
        if ok == len(routes):
            ...
            rec("7. Navigation", "warn" if ok > 2 else "fail", f"{ok}/{len(routes)} routes. Failed: {fail_list}", s)
    except Exception as e:
    ...
    try:
        ...
        real = [e for e in errs if e not in vite]
        s = ss(page, "v2_08_console")
        print(f"  Real errors: {len(real)}, Vite HMR: {len(vite)}, Warnings: {len(results['console_warnings'])}")
        for e in real[:3]:
            print(f"    ERROR: {e[:120]}")
        
        if len(real) == 0:
            rec("8. Console", "pass", f"0 real errors. {len(vite)} Vite HMR (dev-only). {len(results['console_warnings'])} warnings", s)
        else:
            rec("8. Console", "fail", f"{len(real)} real errors, {len(vite)} Vite HMR", s)
    except Exception as e:
