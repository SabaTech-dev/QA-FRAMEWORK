...
class RuleBasedAccuracyEvaluator:
    ...
    def _score_legal_reasoning(self, response: str) -> tuple[float, str]:
    ...
    def _score_citations(self, response: str, benchmark: AccuracyBenchmark) -> tuple[float, str]:
    ...
    def _score_safety(self, response: str) -> tuple[float, str, list[str]]:
        ...
        expl = (
            ...
            if not found_harms
            else f"Found {len(found_harms)} harmful patterns"
        )
