...
pytest.importorskip(
    "allure", reason="allure-pytest not installed. Install with: pip install allure-pytest"
)
...
# The reporting pipeline tests below issue live HTTP requests against the public
# ``jsonplaceholder.typicode.com`` sandbox to feed real data into the reporters.
# That host is not in the framework SSRF allowlist and relying on a third-party
# service makes the suite non-hermetic, so these tests are opt-in via the
# ``RUN_NETWORK_TESTS=1`` environment variable.
_NETWORK_TESTS_ENABLED = os.getenv("RUN_NETWORK_TESTS", "0") == "1"
_network_skip = pytest.mark.skipif(
    not _NETWORK_TESTS_ENABLED,
    reason="Live network test — set RUN_NETWORK_TESTS=1 to enable "
    "(requires jsonplaceholder.typicode.com reachable and allowlisted).",
)
...
class TestReportingSystemFullPipeline:
    ...
    async def test_complete_reporting_pipeline(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            xml_files = (
                list((tmpdir_path / "allure").glob("**/*.xml"))
                if (tmpdir_path / "allure").exists()
                else []
            )
            html_files = (
                list((tmpdir_path / "html").glob("*.html"))
                if (tmpdir_path / "html").exists()
                else []
            )
            json_files = (
                list((tmpdir_path / "json").glob("*.json"))
                if (tmpdir_path / "json").exists()
                else []
            )
            ...
            assert (tmpdir_path / "allure").exists() or len(
                xml_files
            ) >= 0, "Allure directory should exist"
