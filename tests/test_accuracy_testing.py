...
class TestAccuracyEvaluation:
    ...
    def test_compute_overall_accurate(self):
        ev = AccuracyEvaluation(
            criterion_scores=[
                CriterionScore(
                    criterion=EvaluationCriterion.FACTUAL_ACCURACY, score=0.9, explanation=""
                ),
                CriterionScore(
                    criterion=EvaluationCriterion.LEGAL_REASONING, score=0.85, explanation=""
                ),
            ],
        )
    ...
    def test_compute_overall_inaccurate(self):
        ev = AccuracyEvaluation(
            criterion_scores=[
                CriterionScore(
                    criterion=EvaluationCriterion.FACTUAL_ACCURACY, score=0.4, explanation=""
                ),
                CriterionScore(
                    criterion=EvaluationCriterion.COMPLETENESS, score=0.3, explanation=""
                ),
            ],
        )
        result = ev.compute_overall()
...
class TestRuleBasedEvaluator:
    ...
    def test_to_dict_roundtrip(self, evaluator, benchmark_product_liability):
        result = evaluator.evaluate(
            benchmark_product_liability, "AI is liable under ProdHaftG § 1 according to BGH ruling."
        )
        d = result.to_dict()
        assert "id" in d
